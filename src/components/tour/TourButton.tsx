...
export const TourButton: React.FC = () => {
  const { startTour, resetTour, hasSeenTour, isTourOpen } = useTour();
  ...
  const handleTourClick = () => {
    if (hasSeenTour) {
      // Jika user sudah pernah melihat tour, reset dan mulai ulang
      resetTour();
      // Delay sedikit untuk memastikan state ter-reset
      setTimeout(() => {
        startTour();
      }, 1000);
    } else {
      // Jika user belum pernah melihat tour, langsung mulai
      startTour();
    }
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2">      
      ...
      <button
        ...
        title={hasSeenTour ? "Mulai ulang tour aplikasi" : "Mulai tour aplikasi"}
      >
        {hasSeenTour ? <RotateCcw size={24} /> : <HelpCircle size={24} />}
        {/* Tooltip */}

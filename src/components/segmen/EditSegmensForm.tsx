...
export default function EditSegmensForm({ segmen, onSuccess, onCancel }: EditSegmenFormProps) {
  ...
  const [iconLight, setIconLight] = useState(segmen.icon_light || "");
  const [iconDark, setIconDark] = useState(segmen.icon_dark || "");
  const [iconLightPreview, setIconLightPreview] = useState(
    segmen.icon_light ? `/images/product/segmen/${segmen.icon_light}` : ""
  );
  const [iconDarkPreview, setIconDarkPreview] = useState(
    segmen.icon_dark ? `/images/product/segmen/${segmen.icon_dark}` : ""
  );

...
export async function GET(request: Request) {
  ...
  // Validasi kolom yang boleh disort untuk kategoris
  const validSortFields = ["id", "kategori", "created_at"];
  ...
  try {
    ...
    if (search && search.length > 0) {
      whereClause = `WHERE LOWER(kategori) LIKE $1`;
      searchParams_values.push(`%${search}%`);
    }

    // Total count query
    const countQuery = `
      SELECT COUNT(*) FROM public.tbl_kategori
      ${whereClause}
    `;
